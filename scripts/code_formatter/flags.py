...
def flag(flags_dict: Dict[str, Any]) -> Dict[str, Any]:
    """
    Procesa y valida las flags del script code_formatter.

    Valida las configuraciones de entrada para el formateador automático
    ...
    allowed_flags = [
        ...
        'languages',               # Lenguajes a formatear: js,ts,py,md,json,yml (NEW)
    ...
    # Validar que output_location se especifique cuando el formato no es console
    if flags_dict['output_format'] != 'console' and not flags_dict['output_location']:
        raise ValueError(
            f"El formato '{flags_dict['output_format']}' requiere especificar dónde guardar el archivo.\n"
            f"   📁 Usa --output-location=\"ruta/al/archivo.{flags_dict['output_format']}\"\n"
            f"   💡 Ejemplo: --output-location=\"reports/conformance.{flags_dict['output_format']}\"",
        )
